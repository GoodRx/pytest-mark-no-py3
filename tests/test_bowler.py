...
from pytest_mark_no_py3._bowler import filter_already_marked
...
from pytest_mark_no_py3._bowler import remove_marker
from pytest_mark_no_py3._bowler import TestFunc

# Tuples of (code without marker, code with marker)
MODIFIER_TEST_CASES = [
    (
        dedent(
            """\
...
@pytest.mark.parametrize("src, expected", MODIFIER_TEST_CASES)
...
@pytest.mark.parametrize(
    "src, expected",
    [
        (code_with_marker, code_without_marker)
        for code_without_marker, code_with_marker in MODIFIER_TEST_CASES
    ],
)
...
def test_filter_not_already_marked(testdir):
...
def test_filter_already_marked(testdir):
    ...
    expected_testfuncs = [
        TestFunc(str(pyfile), "test_already_marked"),
        TestFunc(str(pyfile), "test_already_marked_many_decorators"),
    ]

    assert filtered_testfuncs(pyfile, filter_already_marked) == expected_testfuncs
